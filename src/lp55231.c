...
void lp_enable_chip(void)
{
	...
	i2c_write_reg_fptr(DEF_LP_ADDR, REG_CNTRL1, &val, 1);
	...
	i2c_write_reg_fptr(DEF_LP_ADDR, REG_MISC, &val, 1);
}

void lp_disable_chip(void)
{
	uint8_t val;
	i2c_read_reg_fptr(DEF_LP_ADDR, REG_CNTRL1, &val, 1);
	val &= ~(1<<CNTRL1_OFFSET_CHIP_EN);
	i2c_write_reg_fptr(DEF_LP_ADDR, REG_CNTRL1, &val, 1);
}
...
void lp_reset_chip()
{
	uint8_t val;
	val = 0xff;
	i2c_write_reg_fptr(DEF_LP_ADDR, REG_RESET, &val, 1);
}
...
lp_err_t lp_set_pwm_by_rgb(uint8_t r, uint8_t g, uint8_t b)
{
	...
	i2c_write_reg_fptr(DEF_LP_ADDR, REG_D1_PWM, vals, 9);
	return LP_ERR_NONE;
}
